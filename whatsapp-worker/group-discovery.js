...
function jidFrom(v){if(!v)return null;const s=String(v);if(s==='@g.us')return null;if(s.endsWith('@g.us'))return s;if(/^\d{8,}(?:-\d{6,})?$/.test(s))return s+'@g.us';return null}
function collectGroupNodes(node,out=new Set()){if(!node||typeof node!=='object')return out;if(node.tag==='group'){const j=jidFrom(node.attrs?.id)||jidFrom(node.attrs?.jid);if(j)out.add(j)}const c=node.content;if(Array.isArray(c))for(const x of c)collectGroupNodes(x,out);return out}
...
for(const row of rows||[]){
  ...
  try{
    ...
    try{await sock.sendPresenceUpdate('available');await wait(3500)}catch(e){presenceError=String(e?.message||e)}
    ...
    if(!closedCode){try{const raw=await sock.query({tag:'iq',attrs:{to:'@g.us',xmlns:'w:g2',type:'get'},content:[{tag:'participating',attrs:{},content:[{tag:'participants',attrs:{}},{tag:'description',attrs:{}}]}]});const rawSet=collectGroupNodes(raw);for(const j of rawSet)candidates.add(j);rawCount=rawSet.size}catch(e){rawError=String(e?.message||e)}}
    ...
    await audit(row.user_id,row.id,{official_count:officialCount,raw_count:rawCount,candidates:candidates.size,verified,failed,official_error:officialError,raw_error:rawError,presence_error:presenceError,closed_code:closedCode});
