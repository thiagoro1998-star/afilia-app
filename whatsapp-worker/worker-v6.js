...
async function upsertGroup(connectionId, userId, jid, name = '', source = '') {
  if (!isGroup(jid)) return;
  const cleanName = String(name || '').trim();
  const { data: old, error } = await db.from('whatsapp_group_refs')
    .select('id,display_name,name_status,is_enabled')
  ...
  if (old) {
    ...
    if (cleanName) {
      patch.display_name = cleanName;
      patch.name_status = 'verified';
      patch.name_source = source || 'event';
      patch.name_verified_at = now();
      patch.last_name_error = null;
    }
    const { error: e } = await db.from('whatsapp_group_refs').update(patch).eq('id', old.id);
    if (e) throw e;
    return;
  }
  const fallback = `Grupo • ${String(jid).replace(/@g\.us$/,'').slice(-4)}`;
  const { error: e } = await db.from('whatsapp_group_refs').insert({
    user_id: userId,
    connection_id: connectionId,
    external_group_ref: jid,
    display_name: cleanName || fallback,
    role: 'destination',
    is_enabled: false,
    name_status: cleanName ? 'verified' : 'pending',
    name_source: cleanName ? (source || 'event') : null,
    name_verified_at: cleanName ? now() : null,
    last_name_error: null
  });
  if (e) throw e;
  log.info({ connectionId, jid, display: cleanName || fallback }, 'group discovered');
}

function wireDiscovery(connectionId, userId, sock) {
  const accept = (jid, name = '', source = '') => {
    if (!isGroup(jid)) return;
    upsertGroup(connectionId, userId, jid, name, source).catch(err => log.warn({ connectionId, jid, source, err: String(err) }, 'group upsert failed'));
  };
  sock.ev.on('messaging-history.set', event => {
    ...
    for (const chat of chats) accept(chat?.id, chat?.name || chat?.displayName || '', 'history');
  ...
  sock.ev.on('connection.update', async update => {
    try {
      ...
      if (update.connection === 'open') {
        ...
        if (requestId) pairResumeMode.delete(requestId);
...
async function fetchImage(url) {
  ...
  const response = await fetch(String(url), {
    redirect: 'follow',
    headers: {
      'user-agent': 'Mozilla/5.0 (Afilia WhatsApp Gateway)',
  ...
  if (buffer.length > 16 * 1024 * 1024) throw new Error('image_too_large');
...
async function processJobs() {
  const { data: jobs = [], error } = await db.from('whatsapp_outbound_jobs')
    .select('id,user_id,connection_id,group_ref_id,offer_id,message_text,attempts,whatsapp_group_refs(external_group_ref,display_name),offers(image_url,product_title)')
  ...
  for (const job of jobs) {
    ...
    let mediaError = null;
    const imageUrl = job.offers?.image_url || null;
    try {
      if (imageUrl) {
        try {
          const media = await fetchImage(imageUrl);
          sent = await session.sock.sendMessage(jid, {
            image: media.buffer,
            caption: job.message_text,
            mimetype: media.mime
          });
          deliveryMode = 'image_caption';
        } catch (err) {
          mediaError = String(err?.message || err).slice(0,220);
          log.warn({ jobId: job.id, jid, mediaError }, 'image delivery failed; using text fallback');
        }
      }
      if (!sent) {
        sent = await session.sock.sendMessage(jid, { text: job.message_text });
        deliveryMode = imageUrl ? 'text_fallback' : 'text';
      }
      await db.from('whatsapp_outbound_jobs').update({
        status:'sent', sent_at:now(), external_message_id:sent?.key?.id||null, updated_at:now(), last_error:null
      }).eq('id',job.id);
...
async function resolvePendingNames() {
  for (const [connectionId, session] of sessions) {
    if (!session.open) continue;
    const due = nameProbeAt.get(connectionId) || 0;
    if (Date.now() < due) continue;
    nameProbeAt.set(connectionId, Date.now() + 45_000);
    const { data: group } = await db.from('whatsapp_group_refs')
      .select('id,user_id,external_group_ref')
      .eq('connection_id', connectionId)
      .neq('name_status','verified')
      .order('updated_at',{ascending:true})
      .limit(1)
      .maybeSingle();
    if (!group?.external_group_ref) continue;
    try {
      const meta = await session.sock.groupMetadata(group.external_group_ref);
      const subject = String(meta?.subject || '').trim();
      if (!subject) throw new Error('metadata_without_subject');
      await db.from('whatsapp_group_refs').update({
        display_name:subject,
        name_status:'verified',
        name_source:'groupMetadata',
      ...
      await db.from('audit_events').insert({
        ...
        event_type:'whatsapp.group_name_probe',
        entity_type:'whatsapp_group_ref',
        entity_id:group.id,
        redacted_metadata:{status:'verified',jid:group.external_group_ref,subject_length:subject.length,at:now()}
      });
    } catch (err) {
      const message = String(err?.message || err).slice(0,220);
      await db.from('whatsapp_group_refs').update({ name_status:'failed', last_name_error:message, updated_at:now() }).eq('id',group.id);
      log.warn({ connectionId, jid:group.external_group_ref, err:message }, 'group name probe failed');
    }
  }
}
...
createServer((req,res)=>{
  ...
  if(req.url==='/'||req.url==='/health'){
    ...
    res.writeHead(200);res.end(JSON.stringify({ok:true,service:'afilia-whatsapp-worker-v6',open,pairing,sessions:sessions.size,at:now()}));return;
  }
...
log.info('Afilia persistent WhatsApp gateway v6 started');
for(;;){
  try{
    await recoverPairRequests();
    await pairWatchdog();
    await restoreConnections();
    await processJobs();
    await resolvePendingNames();
    await heartbeat();
  }catch(err){log.error({err:String(err)},'worker loop failed')}
  if(stopping)break;
  await wait(2500);
}
